...
const getById = async (res: Response, id: number) => {
    const activityGroups = await prisma.$queryRaw<ActivityGroups []>`SELECT * FROM activities WHERE id = ${id}`;
...
const getAll = async ():Promise<ActivityGroups []> => {
    return await prisma.$queryRaw<ActivityGroups []>`SELECT * FROM activities`;
}
...
const deleteById = async (res: Response, id: number): Promise<any> => {
    ...
    return await prisma.$queryRaw`DELETE FROM activities WHERE id = ${id}`;
}
